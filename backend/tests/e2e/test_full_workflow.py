...
Run with: pytest tests/e2e/test_full_workflow.py --e2e
"""
...
from src.summarization.models.correction_signal import CorrectionSignal, CorrectionReasonTag
...
async def round_obj(db_session, discussion):
    ...
    round_data = Round(
        ...
        submission_window_end=datetime.utcnow() + timedelta(minutes=30),
...
def mock_llm_generate():
    ...
    async def _mock_generate(prompt: str, model: str, temperature: float, max_tokens: int):
    ...
    with patch("src.llm.openai_client.generate_summary_llm", side_effect=_mock_generate):
...
class TestCorrectionSignalWorkflow:
    ...
    async def test_correction_signal_final_regeneration(
        ...
        correction = CorrectionSignal(
            correction_signal_id=uuid4(),
            summary_id=summary_2.summary_id,
            reason_tag=CorrectionReasonTag.MISSING_CONTEXT,
    ...
    async def test_correction_signal_final_rejection(
        ...
        correction = CorrectionSignal(
            correction_signal_id=uuid4(),
            summary_id=summary_2.summary_id,
            reason_tag=CorrectionReasonTag.TONE_MISMATCH,
...
class TestApprovalDeadlineWorkflow:
    ...
    Note: This test requires integration with timer service from Spec 0.
    """
...
class TestConstraintsAndPerformance:
    ...
    async def test_max_regeneration_count_enforced(
        ...
        correction = CorrectionSignal(
            correction_signal_id=uuid4(),
            summary_id=summary_2.summary_id,
            reason_tag=CorrectionReasonTag.INACCURATE_REPRESENTATION,
